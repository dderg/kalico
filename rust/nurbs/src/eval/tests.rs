...
fn vector_eval_matches_per_axis_scalar() {
    ...
    for axis in 0..3 {
        ...
        let expected = eval(&scalar, 0.3_f64);
        assert!(
...
fn derivative_of_linear_is_constant() {
    ...
    assert!((eval(&d, 0.5_f64) - 1.0).abs() < 1e-12);
}
...
fn derivative_of_quadratic_at_midpoint_matches_central_difference() {
    ...
    let v = &d;
    let h = 1e-6_f64;
    let expected = (eval(&curve, 0.5 + h) - eval(&curve, 0.5 - h)) / (2.0 * h);
...
fn vector_derivative_matches_per_axis_scalar() {
    ...
    for axis in 0..3 {
        ...
        let expected = eval(&scalar_d, 0.3_f64);
