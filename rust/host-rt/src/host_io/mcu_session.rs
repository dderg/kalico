...
pub struct PendingMcuCall {
    pub completion: SyncSender<Result<McuCallOutcome, TransportError>>,
    pub deadline: Instant,
}

impl std::fmt::Debug for PendingMcuCall {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PendingMcuCall")
...
#[derive(Debug)]
pub struct McuTransportState {
    pub pending: HashMap<u32, PendingMcuCall>,
...
impl Default for McuTransportState {
    fn default() -> Self {
        Self {
            pending: HashMap::new(),
...
pub fn dispatch_mcu_frame(
    ...
    if channel == mcu_transport::CHANNEL_EVENTS || kind.is_event() {
    ...
    if let Some(p) = state.pending.remove(&header.correlation_id) {
        ...
        McuDispatchResult::Ignored
    }
}

fn handle_identify_response(state: &mut McuTransportState, payload: &[u8]) -> McuDispatchResult {
...
fn lift_event_to_runtime_event(
    ...
    match kind {
        MessageKind::FaultEvent => match KFaultEvent::decode(body) {
            Ok(f) => McuDispatchResult::Event(RuntimeEvent::Fault(FaultEvent {
        ...
        MessageKind::McuLog => match KMcuLog::decode(body) {
            Ok(msg) => McuDispatchResult::Event(RuntimeEvent::McuLog(McuLogEvent {
        ...
        MessageKind::EndstopTrip => match KEndstopTrip::decode(body) {
            Ok(t) => McuDispatchResult::Event(RuntimeEvent::EndstopTrip(EndstopTripEvent {
