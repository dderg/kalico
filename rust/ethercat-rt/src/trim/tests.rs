...
fn gains(gain_micro: u32, clamp_um: u16, lpf_millihz: u32, settle_ms: u32) -> TrimGains {
    TrimGains {
        gain_micro,
        clamp_um,
        lpf_millihz,
...
fn armed_bank(gain_micro: u32, clamp_um: u16, lpf_millihz: u32) -> DiffTrimBank {
    ...
    assert_eq!(
        bank.set(4, pair(0, 1), gains(gain_micro, clamp_um, lpf_millihz, 0)),
        0
    );
    bank
}
...
fn set_rejects_bad_slots_gain_clamp_lpf_and_settle() {
    let mut bank = DiffTrimBank::new(CYCLE_NS);
    assert_eq!(
        bank.set(4, pair(2, 2), gains(50_000, 150, 25_000, 0)),
        ERR_TRIM_BAD_SLOT
    );
    assert_eq!(
        bank.set(4, pair(0, 4), gains(50_000, 150, 25_000, 0)),
        ERR_TRIM_BAD_SLOT
    );
    assert_eq!(
        bank.set(
            4,
            pair(0, 1),
            gains(MAX_TRIM_GAIN_MICRO + 1, 150, 25_000, 0)
        ),
        ERR_TRIM_BAD_GAIN
    );
    assert_eq!(
        bank.set(
            4,
            pair(0, 1),
            gains(50_000, MAX_TRIM_CLAMP_UM + 1, 25_000, 0)
        ),
        ERR_TRIM_BAD_CLAMP
    );
    assert_eq!(
        bank.set(4, pair(0, 1), gains(50_000, 150, 50, 0)),
        ERR_TRIM_BAD_LPF
    );
    assert_eq!(
        bank.set(
            4,
            pair(0, 1),
            gains(50_000, 150, MAX_TRIM_LPF_MILLIHZ + 1, 0)
        ),
        ERR_TRIM_BAD_LPF
    );
    assert_eq!(
        bank.set(
            4,
            pair(0, 1),
            gains(50_000, 150, 25_000, MAX_TRIM_SETTLE_MS + 1)
        ),
        ERR_TRIM_BAD_SETTLE
    );
    assert!(!bank.active());
...
fn set_rejects_slot_shared_with_another_pair() {
    ...
    assert_eq!(
        bank.set(4, pair(1, 2), gains(50_000, 150, 25_000, 0)),
        ERR_TRIM_SLOT_IN_USE
    );
    assert_eq!(bank.set(4, pair(2, 3), gains(50_000, 150, 25_000, 0)), 0);
}

#[test]
fn zero_clamp_removes_the_pair_in_either_slot_order() {
    let mut bank = armed_bank(50_000, 150, 25_000);
    assert_eq!(bank.set(4, pair(1, 0), gains(50_000, 0, 0, 0)), 0);
    assert!(!bank.active());
}
...
fn zero_gain_freezes_the_offset_but_keeps_it_applied() {
    ...
    assert_eq!(bank.set(4, pair(1, 0), gains(0, 150, 25_000, 0)), 0);
    let mut offset = vec![0f64; 4];
    for _ in 0..4_000 {
        offset.iter_mut().for_each(|o| *o = 0.0);
...
fn reconfiguring_a_pair_keeps_offset_and_filter_state() {
    ...
    assert_eq!(bank.set(4, pair(1, 0), gains(100_000, 300, 10_000, 100)), 0);
...
fn tightening_the_clamp_reclamps_the_held_offset() {
    ...
    let (_diff, settled) = run_spring(&mut bank, 0.06, true, 40_000);
    assert!(settled < -0.02);
    assert_eq!(bank.set(4, pair(0, 1), gains(50_000, 10, 25_000, 0)), 0);
...
fn settle_window_blinds_the_trim_after_motion() {
    ...
    let mut bank = DiffTrimBank::new(CYCLE_NS);
    assert_eq!(
        bank.set(4, pair(0, 1), gains(50_000, 150, 25_000, settle_ms)),
        0
    );
    let mut offset = vec![0f64; 4];
...
fn reset_clears_offset_filter_settle_and_warning_state() {
    let mut bank = DiffTrimBank::new(CYCLE_NS);
    assert_eq!(
        bank.set(4, pair(0, 1), gains(2_000_000, 50, 25_000, 100)),
        0
    );
    let mut offset = vec![0f64; 4];
    for _ in 0..8_000 {
