...
#[test]
fn a_barrier_waits_for_playback_to_pass_the_fence() {
    let mut lane = SampleLane::new();
    let shared = shared();
    lane.anchor(0, 1_000, 0).expect("anchor accepted");
...
#[test]
fn a_halt_completes_the_fences_it_discarded() {
    let mut lane = SampleLane::new();
    let shared = shared();
    lane.anchor(0, 1_000, 0).expect("anchor accepted");
    push(&mut lane, 0, 0, &[0, 100, 200, 300]);
...
#[test]
fn a_re_anchor_completes_the_fences_it_discarded() {
    let mut lane = SampleLane::new();
    lane.anchor(0, 1_000, 0).expect("anchor accepted");
    push(&mut lane, 0, 0, &[0, 100, 200]);
...
#[test]
fn too_many_unacked_barriers_is_a_loud_fault() {
    let mut lane = SampleLane::new();
    lane.anchor(0, 1_000, 0).expect("anchor accepted");
    push(&mut lane, 0, 0, &[0, 10, 20, 30]);
