...
fn hermite_recovers_general_g2_endpoints() {
    ...
    let got = hermite_g2(anchor([0.0; 3], X, 0.2), anchor(p_b, t_b, -0.3), Z)
        .expect("hermite must converge");
...
fn hermite_zero_boundary_curvature_corner() {
    ...
    let got = hermite_g2(anchor([0.0; 3], X, 0.0), anchor(p_b, t_b, 0.0), Z)
        .expect("hermite must converge");
...
fn hermite_opposite_sign_sharp_apex() {
    ...
    let got = hermite_g2(anchor([0.0; 3], X, 0.46), anchor(p_b, t_b, -0.42), Z)
        .expect("hermite must converge");
...
fn general_blend_rounds_opposite_sign_apex() {
    ...
    let delta = 0.08;
    let blend = solve_general(request(
