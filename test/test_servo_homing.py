...
def test_homing_motor_names_uses_servo_rail_name_when_no_steppers():
    rail = FakeRail(name="servo_x", steppers=[])
...
def make_servo_entries(*specs):
    return [
        {
            "rail": FakeRail(name=name, steppers=[]),
            "handle": handle,
            "slot": slot,
            "limits": limits,
        }
...
def test_post_trip_fault_check_raises_on_fault():
    ...
    with pytest.raises(RuntimeError, match="drive fault 0x8611"):
        homing_mod._check_servo_drive_fault(
            FakeGcmd(error=RuntimeError), engine, 0, entries
        )
    assert engine.calls == [("take_drive_fault", 7)]


def test_post_trip_fault_check_passes_without_fault():
    engine = FakeEngine(take_drive_fault=None)
    entries = make_servo_entries(("servo_x", 7, 0, (8192, 500)))
    homing_mod._check_servo_drive_fault(
        FakeGcmd(error=RuntimeError), engine, 0, entries
    )
...
def test_post_trip_fault_check_skips_non_servo():
    engine = FakeEngine(take_drive_fault=0x8611)
    homing_mod._check_servo_drive_fault(
        FakeGcmd(error=RuntimeError), engine, 0, []
    )
    assert engine.calls == []
...
def run_guarded_trip(engine, se, servo_handle, servo_limits, trip):
    ...
    return homing_mod._run_servo_guarded_trip(
        FakeGcmd(error=RuntimeError), engine, 0, se, servo_rails, trip
    )
...
class FakeHomingRail:
    ...
    def get_steppers(self):
        return [FakeStepper(name="stepper_z")]
...
def run_home_axis(overshoot, retract_dist, positive_dir, servo=False):
    ...
    kin = FakeKin(axis_rails={2: rail}, active_rails_result=[rail])
    ...
    try:
        homer._home_axis(
            ...
            toolhead,
            engine=FakeEngine() if servo else None,
            kin=kin,
            axis=2,
...
def test_active_servo_rails_expand_one_entry_per_motor():
    ...
    entries = homer._active_servo_rails(FakeGcmd(error=RuntimeError), 2, [rail])
