// C-owned MCU structured-log ring + transport. event_log_emit is the only
...
#define EVENT_LOG_HEADER_LEN 7
// McuLog wire body width (messages.rs) — keep in sync.
#define EVENT_LOG_BODY_LEN 24

#define EVENT_LOG_RING_LEN 64
#define EVENT_LOG_RING_MASK (EVENT_LOG_RING_LEN - 1)
...
static volatile struct event_log_entry event_log_ring[EVENT_LOG_RING_LEN];

// Free-running (NOT masked — unsigned wrap is well-defined and head - tail
// gives the live count). Touched only under irq_save.
static volatile uint32_t event_log_head;
static volatile uint32_t event_log_tail;
static volatile uint32_t event_log_seq;
static volatile uint32_t event_log_drops;
...
void
event_log_emit(uint8_t level, uint8_t subsystem, uint16_t event,
                uint16_t code, uint32_t arg0, uint32_t arg1)
{
    irqstatus_t flag = irq_save();
    if ((event_log_head - event_log_tail) >= EVENT_LOG_RING_LEN) {
        event_log_drops++;
        irq_restore(flag);
        return;
    }
    uint32_t idx = event_log_head & EVENT_LOG_RING_MASK;
    event_log_ring[idx].tick = timer_read_time();
    event_log_ring[idx].event = event;
    event_log_ring[idx].code = code;
    event_log_ring[idx].seq = (uint16_t)(event_log_seq & 0xFFFF);
    event_log_ring[idx].level = level;
    event_log_ring[idx].subsystem = subsystem;
    event_log_ring[idx].args[0] = arg0;
    event_log_ring[idx].args[1] = arg1;
    event_log_head++;
    event_log_seq++;
    irq_restore(flag);
}
...
static int
send_log_frame(const struct event_log_entry *e)
{
    uint64_t mcu_tick = widen_log_tick(e->tick);

    uint8_t payload[EVENT_LOG_HEADER_LEN + EVENT_LOG_BODY_LEN];
    payload[0] = (uint8_t)(MCU_MSG_MCU_LOG & 0xFF);
    payload[1] = (uint8_t)((MCU_MSG_MCU_LOG >> 8) & 0xFF);
    payload[2] = EVENT_LOG_MSG_VERSION;
    payload[3] = 0;
    payload[4] = 0;
    payload[5] = 0;
    payload[6] = 0;
    ...
    uint8_t *b = &payload[EVENT_LOG_HEADER_LEN];
...
void
event_log_drain(void)
{
    irqstatus_t df = irq_save();
    uint32_t drops = event_log_drops;
    event_log_drops = 0;
    irq_restore(df);
    if (drops)
        event_log_emit(EVENT_LOG_LEVEL_WARN, EVENT_LOG_SUBSYS_RUNTIME,
                        EVENT_LOG_EVENT_RUNTIME_LOG_DROPS, 0, drops, 0);

    for (;;) {
        struct event_log_entry e;
        irqstatus_t flag = irq_save();
        if (event_log_head == event_log_tail) {
            irq_restore(flag);
            break;
        }
        ...
        e = event_log_ring[event_log_tail & EVENT_LOG_RING_MASK];
        irq_restore(flag);
